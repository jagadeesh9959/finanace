...
export default function FinalSteps({ navigation }) {
  ...
  // Bank details will be populated from AsyncStorage (BasicInfo)
  ...
  useEffect(() => {
    const fetchSavedData = async () => {
      try {
        // fetch loan details (existing)
        const loanJson = await AsyncStorage.getItem("loanDetails");
        if (loanJson) {
          setLoanData(JSON.parse(loanJson));
        }

        // fetch basic info saved from BasicInfo screen
  ...
  // require bank details and the checkboxes to proceed
  const canProceed =
    accountNo.length > 0 &&
    bankName.length > 0 &&
    ifsc.length > 0 &&
    ...
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        >
          ...
          <Text style={styles.checkText}>I agree to the loan terms & policy</Text>
        </TouchableOpacity>
      </View>
      ...
      <TouchableOpacity
        disabled={!canProceed}
        style={[styles.button, { backgroundColor: canProceed ? "#001F54" : "#aaa" }]}
...
const styles = StyleSheet.create({
  ...
  stepTitle: { fontSize: 18, fontWeight: "700", marginBottom: 10 },
  ...
  tick: { fontSize: 16, fontWeight: "bold", color: "#001F54" },
